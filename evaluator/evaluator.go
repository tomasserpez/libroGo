...
func evalInfixExpression(operator string, left, right object.Object) object.Object {
	...
	case left.Type() != right.Type():
		return newError("No coinciden los tipos: %s %s %s", left.Type(), operator, right.Type())
...
//	if !ok {
//		return newError("No es una funcion: %s", fn.Type())
//	}
//
...
func evalIndexExpression(left, index object.Object) object.Object {
	...
	default:
		return newError("No es una index: %s", left.Type())
	}
}
...
func evalHashLiteral(
	...
	for keyNode, valueNode := range node.Pairs {
		...
		if !ok {
			return newError("No se puede usar como hash key. Se obtuvo=%s", key.Type())
		}
...
func evalHashIndexExpression(hash, index object.Object) object.Object {
	...
	if !ok {
		return newError("No es posible utilizarlo como clave: %s", index.Type())
	}
