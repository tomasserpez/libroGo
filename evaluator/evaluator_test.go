...
`, "Operador desconocido: BOOLEAN + BOOLEAN"},
		...
		{
			`{"name": "Monkey"}[fn(x) { x }];`,
			"No es posible utilizarlo como clave: FUNCTION",
		},
	}
...
func TestArrayLiterals(t *testing.T) {
	...
	if !ok {
		t.Fatalf("Object no es Array. Se obtuvo=%T (%+v)", evaluated, evaluated)
	}

	if len(result.Elements) != 3 {
		t.Fatalf("len(result.Elements) no es 3. Se obtuvo=%d", len(result.Elements))
	}
...
false: 6
	...
	if !ok {
		t.Fatalf("Object no es Hash. Se obtuvo=%T (%+v)", evaluated, evaluated)
	}
	...
	if len(result.Pairs) != len(expected) {
		t.Fatalf("Hash tiene la cantidad incorrecta de pares, se obtuvo=%d", len(result.Pairs))
	}

	for expectedKey, expectedValue := range expected {
		pair, ok := result.Pairs[expectedKey]
		if !ok {
			t.Errorf("No hay un par para la clave provista")
		}
